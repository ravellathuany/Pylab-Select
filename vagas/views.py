...
def nova_vaga(request):
    if request.method == "POST":
        ...
        if (len(titulo.strip()) == 0 or len(email.strip()) == 0 or len(experiencia.strip()) == 0 or len(data_final.strip()) == 0 or len(empresa.strip()) == 0 or len(status.strip()) == 0):
...
def mover_tech_dominada(request, id_vaga, id_tech):
    vaga = Vagas.objects.get(id=id_vaga)
    dominada = vaga.tecnologias_dominadas.get(id=id_tech)
    print(dominada)
    vaga.tecnologias_estudar.add(dominada)
    vaga.tecnologias_dominadas.remove(dominada)
    vaga.save()
    return redirect(f'/vagas/vaga/{id_vaga}')

def mover_tech_estudar(request, id_vaga, id_tech):
    vaga = Vagas.objects.get(id=id_vaga)
    estudar = vaga.tecnologias_estudar.get(id=id_tech)
    print(estudar)
